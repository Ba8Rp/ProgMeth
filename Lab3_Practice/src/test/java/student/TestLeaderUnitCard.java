...
public class TestLeaderUnitCard {
	...
	void testSetBuffHealth(){
		...
		assertEquals(55,leaderUnit1.getBuffHealth());
		leaderUnit1.setBuffHealth(-5);
		assertEquals(0,leaderUnit1.getBuffHealth());
//		leaderUnit1.setBuffPower(-3);
//		assertEquals(0,leaderUnit1.getBuffPower());
//		leaderUnit1.setBuffPower(33);
//		assertEquals(33,leaderUnit1.getBuffPower());
	}
	...
	@Test
	void testBuffUnit(){
		UnitCard[] check = unitList;
		oppoUnit1.buffUnit(unitList);
		for (int i=0;i<3;i++){
			assertEquals(check[i].getPower(),unitList[i].getPower());
			assertEquals(check[i].getHealth(),unitList[i].getHealth());
		}
	}
}
