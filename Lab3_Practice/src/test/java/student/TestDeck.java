...
public class TestDeck {
	...
	void testRemoveCard(){
		try {
			Card[] cardNew = Arrays.copyOf(deck3.getDeckList(), deck3.getDeckSize());
			Card ans = deck3.removeCard(3);
			assertEquals(7,deck3.getDeckSize());
			for (int i=0,j=0;i<deck3.getDeckSize();i++,j++){
				if(j==3)j++;
				assertEquals(cardNew[j],deck3.getDeckList()[i]);
			}
			assertNotEquals(NormalUnit2, deck3.getDeckList()[3]);
		}
		...
		try {
			Card[] cardNew = Arrays.copyOf(deck2.getDeckList(), deck2.getDeckSize());
			Card ans = deck2.removeCard(3);

			assertEquals(7,deck2.getDeckSize());
			assertNotEquals(NormalUnit1, deck2.getDeckList()[3]);

			for (int i=0,j=0;i<deck2.getDeckSize();i++,j++){
				if(j==3)j++;
				assertEquals(cardNew[j],deck2.getDeckList()[i]);
			}
		}
		catch (RemoveCardFailedException e){
			throw new RuntimeException(e);
	...
	void testNonExsistanceRemoveCard(){
		assertThrows(RemoveCardFailedException.class, () -> {
			deck3.removeCard(deck3.getDeckSize());
		});
	}
