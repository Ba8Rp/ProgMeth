...
public class CPTSMachine {
    ...
    public static boolean buyTicket(int type, Station start, Station end) {
        ...
        if(t.getStart()==t.getEnd())return false;
        if (isStationExisted(start.getName())&&isStationExisted(end.getName())&&type>=0&&type<3&&(type != 2 || t.getStationDistance(start,end)<=6&&t.isStationValid(start,end))) {
            System.out.println("Bought " +start.getName()+", for " +end.getName()+t.calculatePrice()+" Baht!");
            ticketlist.add(t);
            return true;
        } else {
            System.out.println("This ticket cannot be bought.");
